...
const TestimonialsSection = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="bg-[#111] text-white py-8 px-6 border border-2 border-[#1A1A1A] rounded-lg overflow-hidden"
    >
      <div className="mx-auto">
        {/* Header */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {testimonials.map((item, index) => (
            <div
              key={index}
              className="testimonial-card bg-[#1a1a1a] rounded-xl p-6 border border-gray-800 hover:border-gray-700 transition flex flex-col justify-between"
            >
              {/* Quote */}
              <div>
                <h3 className="text-lg font-semibold mb-3">{item.title}</h3>
