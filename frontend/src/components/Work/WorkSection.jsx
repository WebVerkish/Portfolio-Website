...
import zenith from "../../../public/images/Zenith.png";
import ecommerce from "../../../public/images/ecommerce.png";
import { BlackCircleButton, FilledOrangeSmallButton } from "../Buttons";
...
const WorksSection = () => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(
        ...
        { opacity: 0, y: 100 },
        {
          opacity: 1,
          y: 0,
          stagger: 0.4, // ek ek karke aaye
          ease: "power3.out",
          scrollTrigger: {
            trigger: sectionRef.current,
            start: "top 80%", // jab section 80% viewport par aaye
            end: "bottom 50%",
  ...
  return (
    <section
      ref={sectionRef}
      className="bg-[#111] text-white py-8 px-6 overflow-hidden"
    >
      <div className="mx-auto">
        {/* Header */}
        <div className="flex justify-between items-center mb-8 bg-[#1a1a1a] p-4 rounded-lg shadow-lg">
          <h2 className="text-2xl font-bold">OUR WORKS</h2>
          <button className="flex items-center gap-2 text-sm text-gray-300 hover:text-white transition">
            <BlackCircleButton />
            ALL WORKS
        ...
        <div className="flex flex-col gap-8">
          {works.map((work, index) => (
            <div
              ...
              className="work-card bg-[#1a1a1a] rounded-xl border border-gray-800 hover:border-gray-700 transition overflow-hidden"
            >
              <div className="grid grid-cols-1 md:grid-cols-2">
                {/* Left Side - Info */}
                <div className="p-6 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="text-lg font-semibold">{work.title}</h3>
                      <button className="flex items-center gap-1 text-sm text-gray-300 hover:text-white transition">
                        <BlackCircleButton text="Details" />
                      </button>
                    </div>
                    <p className="text-gray-400 text-sm mb-2 mr-4 bg-[#1f1f1f] inline-block text-gray-300 text-xs px-3 py-1 rounded-lg border border-gray-700">
